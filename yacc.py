...
error_count = 3                # Number of symbols that must be shifted
    ...
    def parse(self,input=None,lexer=None,debug=0):
        ...
        while 1:
            ...
            if t == None:
                ...
                # If we're in this state, the entire parse has been rolled back
                # and we're completely hosed. The token is discarded and we
                # just keep going.
...
def validate_dict(d):
    for n,v in d.items(): 
        ...
        if n[0:2] == 'p_':
            sys.stderr.write(
            	"yacc: Warning. '%s' not defined as a function\n" % n)
        if 1 and isinstance(v,types.FunctionType) and \
        		v.func_code.co_argcount == 1:
            try:
                doc = v.__doc__.split(" ")
                if doc[1] == ':':
                    sys.stderr.write("%s:%d: Warning. Possible grammar rule " +
                    	"'%s' defined without p_ prefix.\n" % (
